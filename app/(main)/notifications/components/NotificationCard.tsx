...
import { HiOutlineChatBubbleLeft, HiOutlineExclamationTriangle, HiOutlineHeart, HiOutlineUser } from 'react-icons/hi2';
...
const NotificationCard = ({notification, index, currentUser}: notificationCardProps) => {

  const notificationTypeMap: Record<string, {message: string, icon:React.ReactNode, href: string }> = {
    'like-post': {
    ...
    'barred-post': {
      message: 'Your post has been barred. It will no longer be seen in public feed.',
      icon: <HiOutlineExclamationTriangle className='size-7 text-primary' />,
      href: `/posts/${notification.post && notification.post._id}`
    },
    'comment-post': {
  ...
  return (
    <AnimationWrapper key={index} transition={{duration: 1, delay: index && index*0}} keyValue={`${index}`}>
      <Link href={href}>
        <article className={cn('flex gap-3 rounded-md bg-card p-4 shadow-sm transition-colors hover:bg-card/70', !notification.read && 'bg-primary/10')}>
          <div className='my-1'>
            {icon}
          </div>
          <div className="space-y-3">
            { notification.type === 'report-post' ? '' : notification.type === 'barred-post' ? '' : <ImageAvatar imgSrc={notification.issuer.image}/> }
            <div>
