...
export const generateMetadata = async ({params: { postId }}: postPageProps):Promise<Metadata> => {
...
const UserInfoSideBar = async ({userInfo}:userInfoSidebarProps) => {
  ...
  return (
    <div className="space-y-4 rounded-md bg-card p-4 shadow-sm">
      ...
      <Link href={`/users/${userInfo.username}`} className='flex items-center gap-3'>
        <ImageAvatar imgSrc={userInfo.image} className='flex-none border'/>
        <div className='text-base'>
          <p className='line-clamp-1 break-all font-semibold hover:underline'>{userInfo.displayName}</p>
          <p className='line-clamp-1 break-all text-muted-foreground'>@{userInfo.username}</p>
        </div>
      </Link>
      <div>
        <div className="flex items-center gap-3">
          <HiOutlineMapPin size={20}/>
          <p className='text-base'>{userInfo.state}, {userInfo.country}</p>
        </div>
        <div className="flex items-center gap-3">
      ...
      { userInfo._id !== loggedInUser._id && (
        <FollowButton userId={userInfo._id} initialState={{
