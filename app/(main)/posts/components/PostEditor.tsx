      ...
      >
        {file.type.startsWith("image") ? (
          <Image
            ...
            width={500}
            height={500}
            className="size-fit max-h-[30rem] rounded-md"
          />
        ) : (
        ...
        {!isUploading && (
          <button
            ...
            className="absolute top-3 right-3 p-1.5 text-background bg-foreground transition-colors rounded-full hover:bg-foreground/60 "
          >
            <HiXMark size={20} />
          </button>
        )}
  ...
  const AttachmentPreviews = ({
    ...
    return (
      <div
        className={cn(
          "w-full h-auto",
          attachments.length > 1 && "columns-2 space-y-4 mx-auto gap-3"
        )}
      >
  ...
  return (
    <div className="flex flex-col rounded-md bg-card p-4 shadow-sm gap-5">
      <div className="flex gap-5 ">
        ...
        <EditorContent
          editor={editor}
          className="w-full max-h-[20rem] overflow-y-auto bg-background rounded-md px-4 py-3"
        />
      </div>
      {!!attachments.length && (
        <AttachmentPreviews
